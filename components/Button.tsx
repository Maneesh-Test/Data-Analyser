...
const Button = React.forwardRef<
  ...
  React.ButtonHTMLAttributes<HTMLButtonElement> & {
    variant?: 'primary' | 'secondary' | 'ghost' | 'teal' | 'teal-gradient' | 'outline';
    size?: 'sm' | 'md' | 'lg';
    isLoading?: boolean;
  }
>(({ className = '', variant = 'primary', size = 'md', isLoading = false, children, ...props }, ref) => {
  ...
  const variantClasses = {
    ...
    outline: 'bg-transparent border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 focus-visible:ring-slate-400',
  };
